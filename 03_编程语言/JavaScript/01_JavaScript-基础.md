...
4. 在字符串中，我们可以使用\作为转义字符，表示一些特殊符号时，可以使用\进行转义；
...
2. JS中可以表示数字的最大值，Number.MAX_VALUE，如果使用Number表示的数值超过最大值，会返回一个Infinity；Infinity表示正无穷，-Infinity表示负无穷，使用typeof检查Infinity返回Number；
3. NaN表示不是一个数值，typeof返回也是Number；



### 布尔值

1. 主要用来做逻辑判断：true表示真，false表示假；
...
3. 如果字符串是空串或是空格的字符，则转换为0；

4. boolean转数字，true→1，false→0；

5. null→数字0；

6. undefined → 数字NaN；

7. **parseInt()** 可以将一个字符串中的有效整数内容去出来，然后转为Number，**parseFloat()**作用类似，不同的是它可以获得有效的小数。
